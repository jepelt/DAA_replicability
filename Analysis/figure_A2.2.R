...
methods <- dtb_h2 %>% distinct(method) %>% arrange(tolower(method)) %>% 
  pull(method)
vlines <- tibble(x = .5 + seq(0, 2 * 24, 2), xend = x, y = 0.5, yend = 58.5)

(pdh <- ggplot(dtb_h2, aes(xn, study)) +
    geom_tile(aes(alpha = std, fill = var)) +
    geom_text(aes(label = value), size = 1.6, vjust = .4) +
    ...
    annotate(geom = 'text', x = -3.5, y = 1:58, vjust = .4, hjust = .5, 
             color = 'grey20', label = typs, size = 1.6) +
    annotate(geom = 'text', x = -2.4, y = 1:58, vjust = .4, hjust = 0,
             color = 'grey20', label = diseases, size = 1.6) +
    annotate(geom = 'text', x = .3, y = 1:58, vjust = .4, hjust = 1,
             color = 'grey20', label = ns, size = 1.6) +
    annotate(geom = 'text', x = c(-4.1, -.2, -2.5), y = 59, vjust = 0,
             hjust = 0, color = 'grey20', label = c('Seq.', 'N', 'Cond.'),
             size = 1.6, fontface = 'bold', angle = 0) +
    annotate(geom = 'text', x = -2.0, y = 1, vjust = 0.4, hjust = 0.5,
             color = 'grey20', label = 'Total', size = 1.6, fontface = 'bold',
             angle = 0) +
    scale_x_continuous(breaks = seq(1.5, 2 * 24 - .5, 2), labels = methods,
                       expand = c(0, 0), limits = c(-4.3, .5 + 2 * 24)) +
    ...
    labs(y = 'Original dataset',
         title = 'Figure A2.2: Number of conflicting and replicated results on split datasets') +
    theme_minimal() +
...
ggsave(filename = paste0('fig_A2.2_', date,'.png'),
