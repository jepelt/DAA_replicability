...
(poph <- ggplot(res_h, aes(method_op, popp)) +
    ...
    geom_hline(yintercept = sgn_level1 * 50, linewidth = 1, linetype = '21',
               color = '#E03E18') +
    coord_flip() +
    scale_y_reverse(expand = c(0, 0), breaks = seq(0, 30, 5),
...
pmainh <- ggplot(tibble(x = 0:1)) +
  labs(title = paste0('Figure A2.1: Results of the split-data analyses')) +
  theme_void() +
...
ggsave(filename = paste0('fig_A2.1_', date,'.png'),
       plot = ph, width = 170, height = 240, dpi = 300, unit = 'mm',
       bg = 'white')
