...
#Summarize results--------------------------------------------------------------
ss_cats <- c('small', 'mid', 'high')
res_summary <- list()

for(i in 1:length(ss_cats)){
  
  res_exploratory <- res_all %>% 
    filter(analysis == 'half' & half == 1 & ss_cat == ss_cats[i]) %>% 
    mutate(q = case_when(method == 'ANCOM-BC2' ~ q05, T ~ q)) %>% 
    filter(q < .05) %>%
    select(method, type, study, iter, taxon, est1 = est, q)
  
  res_validation <- res_all %>% 
    filter(analysis == 'half' & half == 2 & ss_cat == ss_cats[i]) %>% 
    mutate(is2 = T) %>% 
  ...
  res_summary[[i]] <- res_combined %>% 
    ...
    mutate(n = repl + same + opposite + anti + nores,
           ...
           r_repl = repl / n * 100,
           ss_cat = ss_cats[i]) %>% 
    select(method, r_confl, r_repl, r_nhit, ss_cat)
}
...
res_figure <- bind_rows(res_summary) %>%
  ...
  ungroup() %>% 
  select(method, ss_cat, r_confl, r_repl, r_nhit, s_confl, s_repl, s_nhit) %>% 
  pivot_longer(cols = -c(method, ss_cat)) %>% 
  separate(name, into = c('type', 'var')) %>% 
  pivot_wider(names_from = type, values_from = value) %>% 
  mutate(name = factor(var,
                       levels = c('confl','repl', 'nhit'),
                       labels = c('Conflict%', 'Replication%', 'NHits')),
         ss_cat = factor(ss_cat, levels = c('small', 'mid', 'high'))) %>% 
  group_by(method) %>% 
  mutate(total = mean(s)) %>% 
  ungroup() %>% 
...
xlabs <- rep(c('Conflict%', 'Replication%', 'NHits'), 3)
ss <- c('N = 21 - 39', 'N = 40 - 99', 'N = 100 - 374')

(p1 <- ggplot(res_figure, aes(interaction(name, ss_cat), method, fill = s)) +
    geom_tile() +
    geom_text(aes(label = res),
              color = 'grey30', size = 2.7, hjust = 0.55) +
    geom_vline(xintercept = c(3.5, 6.5), color = 'grey20', linetype = '33',
               linewidth = .5) +
    annotate(geom = 'text', x = c(2, 5, 8), y = 15.2,
...
ggsave(filename = paste0('fig_A17_by_sample_size_split_', date,'.png'),
