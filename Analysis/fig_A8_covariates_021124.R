...
res_all <- bind_rows(res_list) %>% 
  ...
  .[, method := ifelse(method == 'MaAsLin2/t-test', 'MaAsLin2\n(lin. regression)', method)] %>%
...
(p1 <- ggplot(res_figure, aes(interaction(alpha, name), method, fill = s)) +
    ...
    geom_vline(xintercept = c(4.5, 8.5), color = 'white',
               linewidth = 1) +
    annotate(geom = 'text', x = 2.5, y = 14.6, vjust = 1, hjust = .5, color = 'grey0',
             label = 'Percentage of conflicting results\n(Conflict%)', size = 2.5) +
    annotate(geom = 'text', x = 6.5, y = 14.6, vjust = 1, hjust = .5, color = 'grey0',
             label = 'Replication percentage\n(Replication%)', size = 2.5) +
    annotate(geom = 'text', x = 10.5, y = 14.6, vjust = 1, hjust = .5, color = 'grey0',
    ...
    labs(#title = 'Figure A8: Results of the split-data analyses with covariates',
         x = x_lab) +
    theme_minimal() +
...
ggsave(filename = paste0('fig_A8_covariates_', date,'.png'),
       plot = p1, width = 170, height = 120, dpi = 300, unit = 'mm',
       bg = 'white')

t <- res_all %>% filter(analysis == 'half') %>% distinct(study)
