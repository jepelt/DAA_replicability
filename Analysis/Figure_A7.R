...
(pclust <- ggplot(dmds, aes(x, y)) +
  ...
  ggrep(aes(label = method), 
        ...
        seed = 1,
        max.time = 2) +
  labs(title = 'Figure A7.1: Some DAA methods clustered based on statistically significant\nresults (53 datasets)') +
...
ggsave(filename = paste0('figure_A7.1_', date,'.png'),
       plot = pclust, width = 170, height = 80, dpi = 300, units = 'mm',
       bg = 'white')
...
(pclust2 <- ggplot(dmds2, aes(x, y)) +
    ...
    ggrep(aes(label = method), 
          ...
          seed = 1,
          max.time = 2) +
    labs(title = 'Figure A7.2: Some DAA methods clustered based on statistically significant\nresults (CDI datasets)') +
    labs(x = paste0('MDS 1 (', eigs[1], '%)'),
         y = paste0('MDS 2 (', eigs[2], '%)')) +
...
ggsave(filename = paste0('figure_A7.2_', date,'.png'),
       plot = pclust2, width = 170, height = 80, dpi = 300, units = 'mm',
       bg = 'white')
