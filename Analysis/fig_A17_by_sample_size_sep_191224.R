...
res_all <- bind_rows(res_list) %>% 
  ...
  mutate(ss_cat = ifelse(n < 70, 'small', 'high'))
...
#Summarize results--------------------------------------------------------------
ss_cats <- c('small', 'high')
res_summary <- list()

for(i in 1:length(ss_cats)){
  
  res_exploratory <- res_all %>% 
    filter(data_id %in% expl_studies & ss_cat == ss_cats[i]) %>% 
...
res_figure <- bind_rows(res_summary) %>%
  ...
  mutate(name = factor(var,
         ...
         ss_cat = factor(ss_cat, levels = c('small', 'high'))) %>% 
  group_by(method) %>% 
  mutate(total = mean(s)) %>% 
  ungroup() %>% 
...
ggsave(filename = paste0('fig_A17_by_sample_size_sep_', date,'.png'),
