...
print(Sys.time())
resl_ancombc2 <- furrr::future_map2(lcounts, lmeta, ~ run_ancombc2(.x, .y),
                                    .options = furrr::furrr_options(seed = 1),
                                    .progress = T)
print(Sys.time())


#save(resl_ancombc2, file = 'res_ancombc_251024.rds')

t <- bind_rows(resl_ancombc2) %>% 
  group_by(data_id) %>% 
  summarize(s = mean(is.na(est)))
