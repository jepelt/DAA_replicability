...
for(i in 1:length(datasets)){
  ...
  res_ldm[[i]] <- run_ldm(counts, meta)
  res_maaslin[[i]] <- run_maaslin2(counts, meta)
}


#logr has to be run last as logistf() may cause problems for some functions
res_logr <- furrr::future_map2(lcounts, lmeta, ~ run_logr_firth(.x, .y),
                               .options = furrr::furrr_options(seed = 1),
                               .progress = T)


res_list <- c(resl, res_ldm, res_maaslin, res_logr)

save(res_list, file = 'res_permuted_301024.rds')