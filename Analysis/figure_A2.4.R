...
vlines <- tibble(x = .5 + seq(0, 2 * 24, 2), xend = x, y = 0.5, yend = 39.5)
...
(pds <- ggplot(dtb_s2, aes(xn, study)) +
    ...
    annotate(geom = 'text', x = -2.0, y = 1, vjust = 0.4, hjust = 0.5, color = 'grey20',
             label = 'Total', size = 1.6, fontface = 'bold', angle = 0) +
    ...
    labs(title = 'Figure A2.4: Number of conflicting and replicated results between separate studies',
...
ggsave(filename = paste0('fig_A2.4_', date,'.png'),
