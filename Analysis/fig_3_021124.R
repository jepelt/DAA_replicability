...
res_figure <- bind_rows(res_bydata, res_sum) %>% 
  ...
  mutate(std = std01(value),
         ...
         disease = ifelse(disease == 'cirrhosis', 'Cirrhosis', disease),
         disease = ifelse(disease == 'CIRR', 'Cirrhosis', disease),
         disease = ifelse(disease == 'parkinson', 'PD', disease),
         disease = ifelse(disease == 'diarrhea', 'Diarrhea', disease),
         disease = ifelse(disease == 'hypertension', 'HT', disease),
         disease = ifelse(disease == 'pre-hypertension', 'PHT', disease),
         disease = ifelse(disease == 'asthma', 'Asthma', disease),
         disease = ifelse(disease == 'schizophrenia', 'Schiz.', disease),
         disease = ifelse(disease == 'migraine', 'Migraine', disease),
         disease = ifelse(disease == 'cephalosporins', 'Ceph.', disease),
         disease = ifelse(disease == 'ME/CFS', 'ME/CFS', disease),
         disease = ifelse(disease == 'EDD', 'CDI', disease),
...
(p1 <- ggplot(res_figure, aes(xn, study)) +
    ...
    geom_text(aes(label = value), size = 2.0, vjust = .4) +
    ...
    annotate(geom = 'text', x = -3.4 - 2, y = 1:58, vjust = .4, hjust = .5, 
             color = 'grey20', label = types, size = ann_size) +
    annotate(geom = 'text', x = -2.5 - 2, y = 1:58, vjust = .4, hjust = 0,
    ...
    annotate(geom = 'text', x = c(-3.9, -2.5, 0.0, 1.7) - 2, y = 59, vjust = 0,
    ...
    scale_x_continuous(breaks = seq(1.5, 2 * 14 - .5, 2), labels = methods,
                       expand = c(0, 0), limits = c(-6.0, .5 + 2 * 14)) +
