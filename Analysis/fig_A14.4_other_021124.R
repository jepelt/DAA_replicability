...
res_combined <- left_join(res_exploratory, res_validation,
  ...
  select(method, type, disease, study1, study2, taxon, sgn1,
...
nvalids <- res_figure %>% group_by(study) %>% slice(1) %>% pull(nvalid)
...
beta_divs <- betad %>% 
  ...
  mutate(study = str_replace(data_id, 'whole 16S |whole sg ', ''),
         r2 = str_sub(round2(r2, 2), 2, 4)) %>% 
  left_join(res_figure %>% group_by(study) %>% slice(1), ., 'study') %>% 
  pull(r2)
  
methods <- res_figure %>% distinct(method) %>% arrange(tolower(method)) %>% 
  pull(method)
vlines <- tibble(x = .5 + seq(0, 2 * 13, 2), xend = x, y = 0.5, yend = 38.5)
...
(p1 <- ggplot(res_figure, aes(xn, study)) +
    ...
    annotate(geom = 'point', x = 10, y = 41.5, color = 'white') +
    annotate(geom = 'text', x = -2.5 - 2, y = 1:38, vjust = .4, hjust = .5, 
             color = 'grey20', label = types, size = ann_size) +
    annotate(geom = 'text', x = -1.8 - 2, y = 1:38, vjust = .4, hjust = 0,
             color = 'grey20', label = condn, size = ann_size) +
    annotate(geom = 'text', x = 1.2 - 2, y = 1:38, vjust = .4, hjust = 1,
             color = 'grey20', label = beta_divs, size = ann_size) +
    annotate(geom = 'text', x = 2.3 - 2, y = 1:38, vjust = .4, hjust = 1,
             color = 'grey20', label = ns, size = ann_size) +
    annotate(geom = 'text', x = c(-5.0, -3.7, -1.9, -.2), y = 39, vjust = 0, hjust = 0, color = 'grey20',
             label = c('Seq', 'Cond\n(NV)', 'Beta', 'N'),
    ...
    labs(#title = 'Figure 14.4: Number of conflicting and replicated results between separate studies',
...
ggsave(filename = paste0('fig_A14.4_other_', date,'.png'),
