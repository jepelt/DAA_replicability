...
for(i in 1:length(alphas)){
  ...
  res_combined <- left_join(res_exploratory, res_validation,
    ...
    filter(!is.na(is2)) %>% 
    group_by(method, study, iter) %>% 
    mutate(q2 = p.adjust(p, method = 'BH')) %>% 
    ungroup() %>% 
    mutate(sgn1 = T,
...
(p1 <- ggplot(res_figure, aes(interaction(alpha, name), method, fill = s)) +
    ...
    labs(#title = 'Figure 2: Results of the split-data analyses',
         x = x_lab) +
    theme_minimal() +
...
ggsave(filename = paste0('fig_A2_fdr_validation_', date,'.png'),
