...
res_figure <- bind_rows(res_bydata, res_sum) %>% 
  ...
  mutate(std = std01(value),
         ...
         disease = ifelse(disease == 'adenoma', 'Adenoma', disease),
...
methods <- res_figure %>% distinct(method) %>% arrange(tolower(method)) %>% 
  pull(method)
vlines <- tibble(x = .5 + seq(0, 2 * 14, 2), xend = x, y = 0.5, yend = 38.5)
...
(p1 <- ggplot(res_figure, aes(xn, study)) +
    ...
    annotate(geom = 'text', x = -3.6 - 2, y = 1:38, vjust = .4, hjust = .5, 
             color = 'grey20', label = types, size = ann_size) +
    annotate(geom = 'text', x = -2.9 - 2, y = 1:38, vjust = .4, hjust = 0,
    ...
    annotate(geom = 'text', x = c(-6.1, -4.8, -1.9, -.2), y = 39, vjust = 0, hjust = 0, color = 'grey20',
             label = c('Seq', 'Condition\n(NV)', 'Beta', 'N'),
    ...
    scale_x_continuous(breaks = seq(1.5, 2 * 14 - .5, 2), labels = methods,
                       expand = c(0, 0), limits = c(-6.2, .5 + 2 * 14)) +
